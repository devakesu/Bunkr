                        ...
                        if (event.isExtra) {
                            return (
                                <div className="flex-shrink-0 w-full sm:w-auto flex items-center justify-end gap-2">
                                    ...
                                    <Button variant="ghost" size="icon" className="h-6 w-6 text-red-400 hover:text-red-500 hover:bg-red-500/10" disabled={isDeleting} onClick={() => handleDeleteTrackData(sessionForDB, event.courseId, dbDate)} aria-label={isDeleting ? "Deleting record" : "Delete record"}>{isDeleting ? <Loader2 className="h-3 w-3 text-primary animate-spin" aria-hidden="true" /> : <Trash2 className="h-3.5 w-3.5" aria-hidden="true" />}</Button>
                                </div>
                            );
                        }
