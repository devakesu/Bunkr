...
const SESSIONS = ["I", "II", "III", "IV", "V", "VI", "VII"];
...
export function AddAttendanceDialog({
  ...
  const getDateKey = (d: Date) => format(d, "yyyyMMdd"); // Standardized using date-fns
  ...
  const handleSubmit = async () => {
    ...
    try {
      const courseObj = coursesData?.courses?.[courseId];
            ...
            <div className="col-span-3">
              <Select value={session} onValueChange={setSession}>
                ...
                <SelectContent className="custom-dropdown max-h-60">
                  {SESSIONS.map((s) => (
                    <SelectItem key={s} value={s}>{s} Hour</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              {isSessionBlocked && (
